...
def register_routes(app):
    ...
    def student_courses():
        ...
        row = cursor.fetchone()
        program = row[0] if row else "BSE"

        # Fetch enrolled courses from program
        cursor.execute("SELECT course_code, title FROM courses WHERE program = ?", (program,))
        all_courses = cursor.fetchall()
        student_courses = [{"code": code, "title": title} for code, title in all_courses]

        # Fetch completed courses
        cursor.execute("SELECT course_code FROM completed_courses WHERE student_id = ?", (student_id,))
        completed = cursor.fetchall()
        completed_courses = [row[0] for row in completed]

        # Fetch prerequisites for each course
        prerequisite_map = {}
        cursor.execute("SELECT course_code, prerequisite_code FROM prerequisites")
        for code, prereq in cursor.fetchall():
            prerequisite_map.setdefault(code, []).append(prereq)
